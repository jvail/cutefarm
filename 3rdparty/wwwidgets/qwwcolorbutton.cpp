...
public:
    QwwColorPopup(ColorModel *model, QWidget *parent = 0) : QWidget(parent, Qt::Popup) {
        ...
        button->setText(QObject::tr("Other"));
...
void _q_colorDialogRequested() {
    ...
    QColor c = QColorDialog::getColor(q->currentColor(), q, QObject::tr("Choose color"), QColorDialog::ShowAlphaChannel);
#else
...
void QwwColorButton::setCurrentColor(const QColor & c) {
    ...
    if (!ind.isValid()) {
        ...
        addColor(c, stdName.isEmpty() ? QObject::tr("Custom color") : stdName);
